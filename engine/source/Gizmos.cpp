...
// ct5037boidengine
// 
// Description of class: Easy Drawing class (provided by UOG)
...
Gizmos::Gizmos(unsigned int a_maxLines, unsigned int a_maxTris)
	: m_programID(glCreateProgram()), m_fragmentShader(glCreateShader(GL_FRAGMENT_SHADER)),
	  m_vertexShader(glCreateShader(GL_VERTEX_SHADER)), m_maxLines(a_maxLines),
	  m_lineCount(0),
	  ...
	  m_tris(new GizmoTri[a_maxTris])
{
	...
	const auto* const vsSource = "#version 150\n \
					 in vec4 Position; \
					 in vec4 Colour; \
					 out vec4 vColour; \
	...
	const auto* const fsSource = "#version 150\n \
					 in vec4 vColour; \
					 out vec4 FragColor; \
	...
	glVertexAttribPointer(1, 4, GL_FLOAT, GL_TRUE, sizeof(GizmoVertex), static_cast<char*>(nullptr) + 16);
                         ...
                         glm::vec3** vertexData /*= nullptr*/, unsigned int* a_vertexCount /*= nullptr*/)
{
	...
	for (unsigned int i = 0; i < a_segments; ++i)
	{
		...
		glm::vec3 v2top(sinf((i + 1) * fSegmentSize) * a_radius, a_fHalfLength,
		                cosf((i + 1) * fSegmentSize) * a_radius);
		...
		glm::vec3 v2bottom(sinf((i + 1) * fSegmentSize) * a_radius, -a_fHalfLength,
		                   cosf((i + 1) * fSegmentSize) * a_radius);
		...
		if (a_filled)
		{
			...
			if (vertexData != nullptr)
			{
				...
				(*vertexData)[index] = a_center + v0top;
				(*vertexData)[index + 1] = a_center + v1top;
				(*vertexData)[index + 2] = a_center + v2top;
				index += 3;
				(*vertexData)[index] = a_center + v0bottom;
				(*vertexData)[index + 1] = a_center + v2bottom;
				(*vertexData)[index + 2] = a_center + v1bottom;
				index += 3;
				(*vertexData)[index] = a_center + v2top;
				(*vertexData)[index + 1] = a_center + v1top;
				(*vertexData)[index + 2] = a_center + v1bottom;
				index += 3;
				(*vertexData)[index] = a_center + v1bottom;
				(*vertexData)[index + 1] = a_center + v2bottom;
				(*vertexData)[index + 2] = a_center + v2top;
			}
		}
...
void Gizmos::addSphere(const glm::vec3& a_center, int a_rows, int a_columns, float a_radius,
                       ...
                       glm::vec3** vertexData/* = nullptr*/, unsigned int* a_vertexCount /*= nullptr*/)
{
	...
	float invColumns = 1.0F / static_cast<float>(a_columns);
	float invRows = 1.0F / static_cast<float>(a_rows);
	...
	auto* v4Array = new glm::vec3[a_rows * a_columns + a_columns];
	...
	for (int face = 0; face < a_rows * a_columns; ++face)
	{
		...
		addLine(v4Array[iNextFace + a_columns], v4Array[face + a_columns], glm::vec4(1.F, 1.F, 1.F, 1.F),
...
void Gizmos::addLine(const glm::vec3& a_rv0, const glm::vec3& a_rv1, const glm::vec4& a_colour0,
...
void Gizmos::draw(const glm::mat4& a_projectionView)
{
		...
		(sm_singleton->m_lineCount > 0 || sm_singleton->m_triCount > 0))
	{
		...
		const unsigned int projectionViewUniform = glGetUniformLocation(sm_singleton->m_programID, "ProjectionView");
		glUniformMatrix4fv(projectionViewUniform, 1, 0u, value_ptr(a_projectionView));
